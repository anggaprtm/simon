...
<x-app-layout>
    ...
    <style>
        ...
        .prodi-wrap * { font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    <div class="py-10 prodi-wrap">
        <div class="max-w-full mx-auto sm:px-6 lg:px-8 space-y-5">

            {{-- PAGE HEADER ROW --}}
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-extrabold text-gray-800">Daftar Unit/Program Studi</h1>
                    <p class="text-sm text-gray-400 mt-0.5">Kelola data program studi atau unit di lingkungan fakultas</p>
                </div>
                <div class="flex items-center gap-2 flex-wrap">
                    <a href="{{ route('program-studi.create') }}" class="top-btn bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm">
                        ...
                        <table class="min-w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left">Nama Unit/Prodi</th>
                                    <th class="text-left">Kode</th>
                                    <th class="text-left">Nama KPS</th>
                                    <th class="text-left">NIP KPS</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="font-bold text-gray-800">{{ $prodi->nama_program_studi }}</div>
                                    </td>
                                    <td>
                                        @if($prodi->kode_program_studi)
                                            <span class="text-xs font-semibold bg-indigo-50 text-indigo-600 px-2 py-1 rounded-md">{{ $prodi->kode_program_studi }}</span>
                                        @else
                                            <span class="text-gray-300">—</span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="font-medium text-gray-700 text-sm">{{ $prodi->nama_kps ?: '—' }}</div>
                                    </td>
                                    <td class="text-gray-500 text-sm font-mono">
                                        {{ $prodi->nip_kps ?: '—' }}
                                    </td>
                                    <td>
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('program-studi.edit', $prodi->id) }}" title="Edit" class="action-btn bg-indigo-50 text-indigo-500 hover:bg-indigo-100">
                                            ...
                                            <form action="{{ route('program-studi.destroy', $prodi->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');" class="inline">
                                                @csrf @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="6" class="py-16 text-center">
                                        <div class="flex flex-col items-center gap-2 text-gray-400">
                                            ...
                                            <p class="text-sm font-medium">Data unit/prodi tidak ditemukan</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
